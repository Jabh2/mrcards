...
class _StudyScreenState extends State<StudyScreen> {
  ...
  bool _correct() {
    ...
    if (q.type == QuestionType.ordering) {
      ...
      final exp = q.answer.split(',').map((e) => e.trim()).toList();
      if (cur.length != exp.length) return false;
      for (var i = 0; i < cur.length; i++) {
        if (normalizeAnswer(cur[i]) != normalizeAnswer(exp[i])) {
          return false;
        }
      }
      return true;
    ...
    if (q.type == QuestionType.matching) {
      ...
      for (final p in q.pairs) {
        final parts = p.split('|');
        if (parts.length < 2) continue;
        final key = normalizeAnswer(parts[0]);
        final val = normalizeAnswer(parts[1]);
        final hit = matchingCurrent.entries.any((e) =>
            normalizeAnswer(e.key) == key &&
            normalizeAnswer(e.value) == val);
        if (!hit) return false;
      }
      return true;
    }
